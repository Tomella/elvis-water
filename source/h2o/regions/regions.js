{
   ...
   class H2oRegionsService {
      constructor($http, $q, configService, mapService) {
         this.$http = $http;
         this.$q = $q;
         ...
         this._features().then(features => {
            this.featuresObj = features;
            this._featuresMap(features);
            this.featuresPromise.resolve(features);
      ...
      */
      catchments() {
         return this.features().then(features => features.map(feature => ({
      ...
      feature(id) {
         return this.features().then(features => this.featuresById[id]);
      }

      _featuresMap(features) {
         if(!this.featuresById) {
            this.featuresById = {};
            features.forEach(feature => {
               this.featuresById[feature.properties.OBJECTID] = feature;
            });
         }
      }
   ...
   angular.module("h2o.regions", ["h2o.select.division", "h2o.select.region"])
      .directive("h2oRegions", ["$http", "h2oRegionsService", "mapService", function ($http, waterRegionsService, mapService) {
         return {
            link: function (scope) {
               let layer;
